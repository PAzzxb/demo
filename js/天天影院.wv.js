...
async function playerContent(flag, id) {
    ...
    let html = resp.body;
    let m3u8 = '';
    let playerMatch = html.match(/var player_aaaa\s*=\s*(\{[\s\S]*?\})\s*(?:;|<\/script>)/);
    if (playerMatch) {
        try {
            let data = JSON.parse(playerMatch[1]);
            if (data.url) {
                m3u8 = data.url.replace(/\\\//g, '/');
            }
        } catch(e) {
            // 解析 JSON 出错时，用正则兜底提取 url 字段
            let urlMatch = playerMatch[1].match(/"url"\s*:\s*"([^"]*)"/);
            if (urlMatch) {
                m3u8 = urlMatch[1].replace(/\\\//g, '/');
            }
        }
    }
