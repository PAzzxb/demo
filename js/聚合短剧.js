...
async function play(flag, id) {
  ...
  // 西饭、星芽等直接返回 id（本身已是播放链接）
  // 注意：星芽的id在detail中已是完整播放链接，此处直接返回
  return JSON.stringify({ parse: 0, url: id });
}
async function search(wd, quick, pg) {
  ...
  try {
    ...
    const headers = await getXingyaHeaders();
    const res = await request(plat.host + plat.search, { method: 'POST', headers, body: JSON.stringify({ text: wd }) });
    const data = (JSON.parse(res)?.data?.theater?.search_data || []).map(i => ({
      vod_id: `星芽@${plat.host}${plat.url2}?theater_parent_id=${i.id}`,
      vod_name: i.title,
      vod_pic: i.cover_url,
      vod_remarks: `星芽短剧｜${i.total}集 播放:${i.play_amount_str}`
    }));
    videos.push(...data.filter(v => v.vod_name?.toLowerCase().includes(wd.toLowerCase())));
  } catch {}
  // 西饭搜索
  try {
