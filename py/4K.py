...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        result = {}
        result = {"class": [   {"type_id": "most-popular", "type_name": "热门影片🌠"},
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        # 处理"分类"页面
        if cid == "categories":
            return self.getCategoryList(pg)
    ...
    # 新增方法：获取全部分类
    def getCategoryList(self, pg):
        videos = []
        try:
            url = f'{xurl}categories/'
            detail = requests.get(url=url, headers=headerx)
            detail.encoding = "utf-8"
            res = detail.text
            doc = BeautifulSoup(res, "lxml")

            section = doc.find('div', class_="list-categories")
            if not section:
                # 尝试其他可能的类名
                section = doc.find('div', class_="categories-list")
                
            if section:
                categories = section.find_all('a', class_='item')
                for cat in categories:
                    # 分类名称
                    name = cat.get('title') or cat.text.strip()
                    ...
                    # 分类图片
                    img = cat.find('img')
                    pic = img.get('src') if img else img.get('data-src') if img else ''
                    if pic and 'http' not in pic:
                        if pic.startswith('//'):
                            pic = 'https:' + pic
                        elif not pic.startswith('/'):
                    ...
                    # 视频数量
                    count_span = cat.find('span', class_='videos')
                    if not count_span:
                        count_span = cat.find('div', class_='videos')
                    count = count_span.text.strip() if count_span else '0'
                    
                    # 构造分类项
                    video = {
                        "vod_id": href,  # 使用相对路径作为ID
                        "vod_name": name,
                        "vod_pic": pic,
                        "vod_remarks": f"{count}部"
                    }
                    videos.append(video)

        except Exception as e:
            print(f"Error in getCategoryList: {str(e)}")
