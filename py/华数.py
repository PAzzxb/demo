...
class Spider(Spider):
    ...
    def get_current_app_key(self,xurl, headerx):
        ...
        match = re.search(js_path_pattern, resp.text)
        ...
        target_key_b64 = key_match.group(2)
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        NdType = ext['年代'] if '年代' in ext else '全部'

        DqType = ext['地区'] if '地区' in ext else '全部'

        LxType = ext['类型'] if '类型' in ext else '全部'
        ...
        for vod in data['data']:
            ...
            id = f"{vod['nodeId']}@{vod['newsId']}"

            pic = vod['hPic']

            year = vod.get('pubTime', '暂无日期')

            remark = vod.get('episodeDesc', '暂无备注')
    ...
    def detailContent(self, ids):
        ...
        content =  data.get('data', {}).get('newsAbstract', '')

        director = data.get('data', {}).get('director', '')

        actor = data.get('data', {}).get('actor', '')

        remarks = data.get('data', {}).get('episodeDesc', '')

        year = data.get('data', {}).get('pubTime', '')

        area = data.get('data', {}).get('countryTag', '')
    ...
    def get_x_sign_for_post(self,secret_b64,playUrl):
        ...
        data_string = json.dumps(payload_dict, separators=(',', ':'), ensure_ascii=False)
        signature = hmac.new(secret_key, data_string.encode('utf-8'), hashlib.sha256).digest()
        x_sign = base64.b64encode(signature).decode('utf-8')
        return x_sign
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        id = id.replace('.mp4', '/playlist.m3u8')
